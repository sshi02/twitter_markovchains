...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null) {
            throw new IllegalArgumentException("addBigram: first parameter is null");
        }
    ...
    public void train(Iterator<String> sentence) {
        if (sentence == null) {
            throw new IllegalArgumentException("train(): given Iterator sentence is null");
        }
        if (sentence.hasNext()) {
            String last = sentence.next();
            if (!last.equals("")) {
                startWords.record(last);
    ...
    @Override
    public String next() {
        String currString = nextString;
        if (!hasNext()) {
            throw new NoSuchElementException("next(): no more words on the walk");
        }
        ProbabilityDistribution<String> pd = chain.get(currString);
        if (pd != null) {
            nextString = pd.pick(ng);
        } else {
            nextString = null;
        }
        return currString;
