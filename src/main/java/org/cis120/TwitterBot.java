...
public class TwitterBot {
    ...
    public String generateTweet(int numWords) {
        this.mc.reset();
        if (numWords < 0) {
            throw new IllegalArgumentException("generateTweet(): integer input numWords is null");
        } else if (numWords == 0) {
            return "";
        }
        ...
        if (mc.hasNext()) {
            while (currNum < numWords) {
                String word = mc.next();
                if (!mc.hasNext() && 1 + currNum < numWords) {
                    tweet += word + randomPunctuation() + " ";
                    currNum++;
                    mc.reset();
                } else if (1 + currNum < numWords) {
                    tweet += word + " ";
                    currNum++;
                } else {
                    tweet += word;
